...
<head>
            ...
            success : function(data){
                ...
                if(data.length > 0){
                    for(i=0; i<data.length; i++){
                        	html += "<table class='table'><h6><strong>"+ data[i].reply_writer+"</strong></h6>";
                            html += data[i].reply_num+ "  ";
                            html += "<div class = 'media text-muted pt-3' id= 'comment"+data[i].reply_num+"'>";
                            html += data[i].reply_content;
                            html += "<button class='btn btn-xs btn-link' onClick= 'cm_updateForm("+ data[i].reply_num+ ")'>수정2</button>";
                            html += "<button  class ='btn btn-xs btn-link' onClick= 'cm_delete("+data[i].reply_num+")'>삭제</button>";
                            html += "<tr><td>";
                            html +=	"</td></tr>";
                            html+= "</div>";
                            html += "</table>";
	...
	function cm_updateForm(reply_num){
		...
		html+= "<button type='button' onClick='reply_updateComment("+ reply_num+ ")'>수정</button>";
	...
	function reply_updateComment(reply_num){
		var reply_reComment = document.getElementsByName("reply_reComment"+ reply_num)[0].value;
		var json= {"reply_num":reply_num, "reply_reComment":reply_reComment};
...
<div class="container">
    ...
    <br><br>
        <div>
            <div>
                <span><strong>Comments</strong></span><p>게시판번호 : ${view.board_num}</p>댓글수 : <span id="commentCount"></span>
                <input type="hidden" id="board_num" name="board_num" value="${view.board_num}" />  
            </div>
            <div>
                <table class="table">                    
                    <tr>
                        <td>
                            <textarea style="width: 1100px" rows="3" cols="30" id="reply_content" name="reply_content" placeholder="댓글을 입력하세요"></textarea>
                            <br>
                            <div>
                                <a href='#' onClick="cm_insert('${view.board_num}')" class="btn pull-right btn-success">등록</a>
                                <%-- <a href='#' onClick ="cm_modal('${comment.reply_num}')" class="btn pull-right btn-success">수정1</a> --%>
                                <%-- <a href='#' onClick ="cm_modal('${comment.reply_num}')" class="btn pull-right btn-success">수정1</a> <!-- 리플값테스트 -->
                                <button type='button' class='btn pull-right btn-sm btn-warning' data-toggle='modal'  data-target='#modifyModal'>수정2</button>
                                <button type='button'id='cm_modal' class='btn pull-right btn-sm btn-warning' data-toggle='modal' data-target='#modifyModal'>수정3</button> --%>
                                <!-- /board/commentModal&reply_num=${reply_num} -->	<!-- ${reply_num} -->
                            </div>
                        </td>
                    </tr>
                </table>
